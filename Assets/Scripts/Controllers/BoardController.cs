...
public class BoardController : MonoBehaviour
{
    ...
    private YieldInstruction wait0_2;
    private YieldInstruction wait0_3;
    ...
    private void Start()
    {
        rayMouse = new RaycastHit2D[1];
        wait0_2 = new WaitForSeconds(0.2f);
        wait0_3 = new WaitForSeconds(0.3f);
    }
    ...
    public void Update()
    {
        ...
        {
            var hit = Physics2D.RaycastNonAlloc(m_cam.ScreenToWorldPoint(Input.mousePosition), Vector2.zero, rayMouse);
            if (hit > 0)
            {
                m_isDragging = true;
                m_hitCollider = rayMouse[0].collider;
            }
        }
        ...
        {
            var hitCount = Physics2D.RaycastNonAlloc(m_cam.ScreenToWorldPoint(Input.mousePosition), Vector2.zero, rayMouse);
            if (hitCount > 0 )
            {
                var hit = rayMouse[0];
