...
public class Board
{
    ...
    {
        var collectionOrderItemNormal = CollectionOrderItemNormal();
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem item = new NormalItem();
                var suroundCells = cell.SurroundCells;
                int indexTypeMin = -1;
                for(int i  = 0; i < collectionOrderItemNormal.Count; ++i)
                {
                    indexTypeMin = collectionOrderItemNormal[i];
                    var existCellType = false;
                    for(int j = 0; j < suroundCells.Length; ++j)
                    {
                        var suroundCell = suroundCells[j];
                        if (suroundCell == null || suroundCell.IsEmpty) continue;
                        if (suroundCell.Item is NormalItem normalItem)
                        {
                            if((int)normalItem.ItemType == indexTypeMin)
                            {
                                existCellType = true;
                                break;
                            }
                ...
                item.SetType( (NormalItem.eNormalType)indexTypeMin);
                item.SetView();
                item.SetViewRoot(m_root);
    ...
    List<int> CollectionOrderItemNormal()
    {
        var collection = new List<int>() { 0, 1, 2, 3, 4, 5, 6 };
        collection.Sort(CompareSortOrderItemType);
        return collection;
    }

    internal int CompareSortOrderItemType(int indexA,int indexB)
    {
        return CountItemTypeOnBoard((NormalItem.eNormalType)indexA).CompareTo(CountItemTypeOnBoard((NormalItem.eNormalType)indexB));
    }

    internal int CountItemTypeOnBoard(NormalItem.eNormalType type)
    {
        int count = 0;
        for (int x = 0; x < boardSizeX; x++)
        {
